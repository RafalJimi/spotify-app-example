import React from "react";
import { ContextMenu, MenuItem, ContextMenuTrigger } from "react-contextmenu";
import { Menu } from "./components/Menu/index";
import { SongData } from "./index";
import {
  ContextContainer,
  SongItemContainer,
  Number,
  PlayButton,
  AlbumIMG,
  SongTitle,
  FavButton,
  Time,
  OptionsMenu,
  Option,
  PlaylistsContainer,
  Playlist,
} from "./layout.styled";

type SongsListItemLayoutProps = {
  id: number;
  songData: SongData;
};

export const SongItemLayout = ({ id, songData }: SongsListItemLayoutProps) => (
  <ContextContainer>
    <ContextMenuTrigger id={`ShowSectionItemMenu-${id}`}>
      <SongItemContainer>
        <Number className="number">{id}</Number>
        <PlayButton className="playButton">
          <i className="fas fa-play"></i>
        </PlayButton>
        <AlbumIMG>
          <img
            src={
              "data:image/jpeg;base64,"
            }
          />
        </AlbumIMG>
        <SongTitle>{songData.trackName}</SongTitle>
        <FavButton className="favButton">
          <i className="far fa-heart"></i>
        </FavButton>
        <Time>{songData.trackLenght}</Time>
        <Menu artist={songData.artistName} album={songData.albumName} />
      </SongItemContainer>
    </ContextMenuTrigger>

    <ContextMenu id={`ShowSectionItemMenu-${id}`}>
      <MenuItem>
        <OptionsMenu isOpen={true}>
          <Option>Go to album</Option>
          <Option>
            <span>Add to playlist</span> <i className="fas fa-angle-right"></i>
            <PlaylistsContainer>
              <Playlist>Test</Playlist>
              <Playlist>Test 2</Playlist>
            </PlaylistsContainer>
          </Option>
        </OptionsMenu>
      </MenuItem>
    </ContextMenu>
  </ContextContainer>
);
